...
class MongoDBTable:
    ...
    def insert_c(self, new_comment):
        ...
        #k = self._get_key_string(new_record)
        #new_record["primary_key"] = k
        res = self._collection['children'].insert_one(new_comment).inserted_id
        return res
    ...
    def update_forum(self, template, new_values):
        """
        ...
        # parent_id = forum_name['post_id']
        # object = forum_name['post_id']['children']
        # #res = self._collection.update({"parent_id": }, new_values)
        # return res
        res = self._collection.update(template, {"$set": new_values})
        return res

    def find_by_query(self, post_id, children_search, field_list=None):
        selected_cols = {"post_id": 1, "children": 1}

        query = [{'$match': post_id},
                 {'$unwind': {'path': '$children'}},
                 {'$match': children_search},
                 {'$project': selected_cols}]

        res = self._collection.aggregate(query)
        list_obj = []
        for r in res:
            list_obj.append(r)
        return list_obj
